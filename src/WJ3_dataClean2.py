...
'''读入文件'''
def ReadFile():
    df1 = pd.read_csv('./1Data_csv/data3.csv')
...
if __name__ == '__main__':
    '''下面的代码是在新文件./1Data_csv/data3.csv上执行的'''
    ...
    # (2) 最大加速度：(100*1000)/(7*3600) = 3.96825 m/s^2      (100-0)/(7/3600)=51428.57km/h^2       ；最大减速度：7.5~8 m/s^2    (7.5/1000)/(1/(3600*3600))=97200km/h^2 ~ (8/1000)/(1/(3600*3600))=103680km/h^2
    Max_A_D()
    # WriteFile('./3MidData_csv3/data3_(Max_A_D)_YESnan_' + str(df1.shape[0]) + '.csv')
    df1 = df1.dropna()
    WriteFile('./3MidData_csv3/data3_(Max_A_D)_NOnan_' + str(df1.shape[0]) + '.csv')
    # print(df1[["时间", "GPS车速", "瞬时加速度"]])
