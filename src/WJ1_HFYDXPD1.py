...
def ReadFile():
    #17-12-18~17-12-24
    df1 = pd.read_csv('./3MidData_csv/data1_(DropGPSV10)_177603.csv')
...
def ChooseMotionSeries():
    ...
    f3=(LianXuMotionDuan_List['怠速秒数']/LianXuMotionDuan_List['总秒数'])<=0.5#怠速占总时间 不超过90%
...
if __name__ == '__main__':
    '''下面的代码是在data1_(DropGPSV10)_177603.csv上执行的'''
    ...
    #划分运动学片段。
    HFMotionSeries()
    LianXuMotionDuan_List.to_csv('./3MidData_csv/未筛选的运动学片段_'+str(len(LianXuMotionDuan_List))+'_(50%).csv',index=False)

    # 筛选运动学片段。
    ChooseMotionSeries()
    LianXuMotionDuan_List.to_csv('./3MidData_csv/筛选后的运动学片段_' + str(len(LianXuMotionDuan_List)) + '_(50%).csv', index=False)

    # 运动学片段的特征
    MotionSeriesFeature()
    MotionSeriesFeature_List.to_csv('./3MidData_csv/MotionSeriesFeature_'+str(len(MotionSeriesFeature_List))+'_(50%).csv',index=False)
