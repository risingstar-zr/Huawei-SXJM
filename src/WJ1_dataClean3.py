...
def ReadFile():
    #17-12-18~17-12-24
    df1 = pd.read_csv('./3MidData_csv/data1_(Max_A_D)_NOnan_184971.csv')
...
def GPS_V0_EveryHowLong():
    ...
    # 画图
    DrawPic(df_LianXuDuan['起始时刻'].tolist(), df_LianXuDuan['记录数'].tolist(), '起始时刻', '持续时间(s)', 'GPS车速为0的连续时间段规律')
...
if __name__ == '__main__':
    '''下面的代码是在新文件data1_(Max_A_D)_NOnan_184971.csv上执行的'''
    ...
    DropYuanShiFile()
    df1.to_csv('./3MidData_csv/data1_(DropGPSV0)_' + str(len(df1)) + '.csv', index=False)
    # GPS_V0_EveryHowLong()
