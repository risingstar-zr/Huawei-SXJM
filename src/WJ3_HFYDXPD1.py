...
'''读入文件'''
def ReadFile():
    df1 = pd.read_csv('./3MidData_csv3/data3_(DropGPSV10)_157846.csv')
...
if __name__ == '__main__':
    '''下面的代码是在data3_(DropGPSV10)_157846.csv上执行的'''
    ...
    #划分运动学片段。
    HFMotionSeries()
    LianXuMotionDuan_List.to_csv('./3MidData_csv3/未筛选的运动学片段_'+str(len(LianXuMotionDuan_List))+'_(80%).csv',index=False)

    # 筛选运动学片段。
    ChooseMotionSeries()
    LianXuMotionDuan_List.to_csv('./3MidData_csv3/筛选后的运动学片段_' + str(len(LianXuMotionDuan_List)) + '_(80%).csv', index=False)

    # 运动学片段的特征
    MotionSeriesFeature()
    MotionSeriesFeature_List.to_csv('./3MidData_csv3/MotionSeriesFeature_'+str(len(MotionSeriesFeature_List))+'_(80%).csv',index=False)
