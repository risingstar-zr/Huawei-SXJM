...
'''读入文件'''
def ReadFile():
    df1 = pd.read_csv('./3MidData_csv2/data2_(DropGPSV0)_143054.csv')
...
def GPS_V10_EveryHowLong():
    ...
    # 画图
    DrawPic(df_LianXuDuan['起始时刻'].tolist(), df_LianXuDuan['记录数'].tolist(), '起始时刻', '持续时间(s)', 'GPS车速大于等于0小于10的连续时间段规律2')
...
if __name__ == '__main__':
    '''下面的代码是在data2_(DropGPSV0)_143054.csv上执行的'''
    ...
    DropYuanShiFile()
    df1.to_csv('./3MidData_csv2/data2_(DropGPSV10)_' + str(len(df1)) + '.csv', index=False)
